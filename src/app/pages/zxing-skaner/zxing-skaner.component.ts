...
@Component({
  selector: 'app-zxing-skaner',
  templateUrl: './zxing-skaner.component.html',
  styleUrl: './zxing-skaner.component.scss',
  standalone: false
})
export class ZxingSkanerComponent implements OnInit, OnDestroy {

  /**
   * Supported Hints map.
   */
  private _hints: Map<DecodeHintType, any> | any;

  /**
   * The ZXing code reader.
   */
  private _codeReader: BrowserMultiFormatContinuousReader|any;

  /**
   * The device that should be used to scan things.
   */
  private _device: MediaDeviceInfo|any;
   ...
   *
   */
  private _isAutostarting: boolean|any;
   ...
   */
  private hasPermission: boolean | any;
  ...
  previewElemRef: ElementRef<HTMLVideoElement>|any;
  ...
  private _devicePreStart: MediaDeviceInfo|any;
   ...
   */
  @Input()
  set device(device: MediaDeviceInfo | any) {
  ...
  async askForPermission(): Promise<boolean> {
    ...
    try {
      ...
      return this.handlePermissionException(err as DOMException);
    } finally {
  ...
  private async autostartScanner(devices: MediaDeviceInfo[]): Promise<void> {
    ...
    const matcher = ( label:any ) => /back|trás|rear|traseira|environment|ambiente/gi.test(label);
  ...
  private async scanFromDevice(deviceId: string): Promise<void> {
    ...
    const next = (x: ResultAndError) => this._onDecodeResult(x.result as Result, x.error as Exception);
   ...
   */
  private _reset(): MediaDeviceInfo|undefined {
  ...
  public reset(): void {
    this._reset();
    this.deviceChange.emit(undefined);
  }
   ...
   */
  private setPermission(hasPermission: boolean): void {
